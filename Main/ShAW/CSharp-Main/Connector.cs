...
using Newtonsoft.Json; // install via NuGet or manually include JSON.NET
...
namespace shaw
{
    public class Connector : MonoBehaviour
    {
        ...
        private async void Start()
        {
            ...
            Plugin.Log?.Debug($"Starting with Node ID: {nodeId}");
        ...
        private async Task ConnectAndRun()
        {
            while (!cts.Token.IsCancellationRequested)
            {
                try
                {
                    ...
                    Plugin.Log?.Debug("[UnityClient] Connected via TCP");
                ...
                {
                    Plugin.Log?.Debug($"Connection error: {e.Message}, retrying...");
        ...
        private string GenerateHash(string input)
        {
            ...
            {
                var bytes = Encoding.UTF8.GetBytes(input + UnityEngine.Random.Range(0, 1000000));
                    ...
                    if (response.Contains("101 Switching Protocols"))
                    {
                        foreach (var line in response.Split('\n'))
                        {
                            if (line.StartsWith("Upgrade-Port:"))
                            {
                                ...
                                {
                                    Plugin.Log?.Debug($"Upgraded to port {port}");
                                    return port;
                                }
                            }
                        }
        ...
        private async Task SendJson(object obj)
        {
            ...
            {
                Plugin.Log?.Debug($"SendJson error: {e.Message}");
            }
        }
            ...
            using (var reader = new StreamReader(stream, Encoding.UTF8))
            {
                while (!cts.Token.IsCancellationRequested)
                {
                    try
                    {
                        ...
                        {
                            case "forward":
                                Plugin.Log?.Debug($"Forward command received: {msg}");
                                await HandleForward(msg);
                                break;

                            default:
                                Plugin.Log?.Debug($"Unknown message: {line}");
                                break;
                        }
                    }
                    catch (Exception e)
                    {
                        Plugin.Log?.Debug($"Listen error: {e.Message}");
        ...
        private async Task HandleForward(Dictionary<string, object> msg)
        {
            try
            {
                ...
                {
                    Plugin.Log?.Debug("Forward command missing required fields.");
                    return;
                }
                ...
                Plugin.Log?.Debug($"Forward setup ({direction}) local={localPort}, remote={remotePort}");
        ...
        private async Task StartLocalForward(int localPort, int remotePort)
        {
            try
            {
                ...
                listener.Start();
                Plugin.Log?.Debug($"Local forward listening on {localPort}");
            ...
            {
                Plugin.Log?.Debug($"Local forward error: {e.Message}");
            }
        }
        ...
        private async Task HandleLocalConnection(TcpClient localClient, int remotePort)
        {
            try
            {
                ...
                using (var remoteClient = new TcpClient())
                {
                    ...
                    Plugin.Log?.Debug($"Connected remote port {remotePort}");
            ...
            {
                Plugin.Log?.Debug($"Connection forward error: {e.Message}");
            }
        }
        ...
        private async Task StartRemoteForward(int localPort, int remotePort)
        {
            try
            {
                using (var localClient = new TcpClient())
                {
                    ...
                    {
                        await remoteClient.ConnectAsync(ServerAddress, remotePort);
                        Plugin.Log?.Debug($"Remote forward connected {localPort} -> {remotePort}");
            ...
            {
                Plugin.Log?.Debug($"Remote forward error: {e.Message}");
            }
        }
            ...
            while (!cts.Token.IsCancellationRequested)
            {
                try
                {
                    var health = new
                    {
                        ...
                        status = "healthy",
                        uptime = Time.time,
                        RAM_usage = $"{SystemInfo.systemMemorySize} MB",
                        CPU_usage = SystemInfo.processorCount + " cores"
                    };
                ...
                {
                    Plugin.Log?.Debug($"Health loop error: {e.Message}");
                    await Task.Delay(5000);
                }
            }
        }
